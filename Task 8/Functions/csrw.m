%
%   	CSRW.M
% This Matlab performs the two-pass CSR with a fixed
% weighting matrix.
%
% Input:
...
% W: weighting matrix (default is I_N)
%
% Output:
% R2: Sample CSR R^2
...
function [R2,pval1a,pval1b,pval2a,pval2b,pval3a,pval3b,rse,gamma,trat1,trat2,trat3,trat4, ...
          lambda,trat1a,trat2a,trat3a,trat4a,csrta,pval4a,pval5a,csrtb,pval4b,pval5b] = csrw(R,F,lag,W)
[T,N] = size(R);
if nargin<3
   lag = 0;
end
...
ht0 = gammat+(zt*H).*(ut*ones(1,K+1));   % Imposing H0: gamma_1=0_K
...
%for i = 1:K+1
%[hh,pp,statt,cv] = lbqtest(ht0(:,i),1);
%pp
%end
%
...
ht2 = ht1+([zeros(T,1) Fd]*H1).*(ut*ones(1,K+1));        % With misspecification adjustment
V1 = nw(ht1,lag);
V2 = nw(ht2,lag);
